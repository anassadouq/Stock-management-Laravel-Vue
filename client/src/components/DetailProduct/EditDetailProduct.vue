...
const fetchDetailProduct = async () => {
    try {
        const { data } = await axios.get(`http://127.0.0.1:8000/api/detail_product/${form.product_id}`);
        detailProduct.value = data;
        form.fournisseur_id = data.fournisseur_id; // Assuming the response contains a fournisseur_id
        form.qte = data.qte;
        form.pu = data.pu;
...
// Fetch fournisseurs and detail product data
onMounted(() => {
    fetchFournisseurs();
    fetchDetailProduct();
});
...
const updateDetailProductMutation = useMutation({
    mutationFn: async (updatedDetailProduct) => {
        await axios.put(`http://127.0.0.1:8000/api/detail_product/${form.product_id}`, updatedDetailProduct);
    },
    onSuccess: () => {
        toast.success('Achat mis à jour avec succès');
        router.push(`/detailProduct/show/${form.product_id}`);
    },
    onError: () => {
        ...
        <div class="container m-auto max-w-2xl py-20">
            <div class="bg-white px-6 py-8 mb-4 shadow-md rounded-md border m-4 md:m-0">
                <form @submit.prevent="handleSubmit">
                    <h2 class="text-3xl text-center font-semibold mb-6">Modifier le detail produit</h2>
